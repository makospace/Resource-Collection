...
struct LSH{//离散化
	...
	inline void run(){
		auto it=s.begin(),ti=s.end();  //离散化之后的编号是从1开始的，如果是不连续的，cnt也要不连续
		for(;it!=s.end();it++){
			if(it!=s.begin()){
...
#define ll long long
ll MIN(ll a,ll b){return a==0?b:b==0?a:a<b?a:b;} //注意这里的0是无穷大
#define ls (p<<1)
#define rs (p<<1|1)
#define mid (l+r>>1)
...
struct node{//下面是线段树
	...
	node operator = (ll b){return {sum=b*len,mn=b,lazy=b,len};} //len不变 
}tr[800005];
...
void chg(int p,int l,int r,int L,int R,ll c){
	...
	pushdown(p);  //往下进行操作的时候，要先把懒标记传递下去，因为有效直接更新就是在子节点上，所以要先pushdown，父节点永远是通过子节点更新的
	if(L<=mid) chg(ls,l,mid,L,R,c);  
	if(R>mid) chg(rs,mid+1,r,L,R,c);
	tr[p]=tr[ls]+tr[rs];  //相当于pushup
}
